<?php
    ...
    if(isset($_POST) && !empty($_POST)){
        ...
        $h['isStock'] = (isset($_POST['form_isStock'])?1:0);
        $h['isBlock'] = (isset($_POST['form_isBlock'])?1:0);
        $h['isDefault'] = (isset($_POST['form_isDefault'])?1:0);
    ...
    if (isset($_GET['id_statut']) && !empty($_GET['id_statut'])) {
        ...
        $data_statut['isStock'] = 0;
        $data_statut['isBlock'] = 0;
        $data_statut['isDefault'] = 0;
    }
    ...
    // Statut par defaut
    if($data_statut['isDefault'])
        $html .= '               <label>Statut par defaut ? <input type="checkbox"  name="form_isDefault" value="1" checked="checked"/></label>';
    else
        $html .= '               <label>Statut par defaut ? <input type="checkbox"  name="form_isDefault" value="1" /></label>';

    // Stock
    if($data_statut['isStock'])
        $html .= '               <label>Décrémentation Stock ? <input type="checkbox"  name="form_isStock" value="1" checked="checked"/></label>';
    else
        $html .= '               <label>Décrémentation Stock ? <input type="checkbox"  name="form_isStock" value="1" /></label>';

    // Bloqué
    if($data_statut['isBlock'])
        $html .= '               <label>Statut Bloquant ? <input type="checkbox"  name="form_isBlock" value="1" checked="checked"/></label>';
    else
        $html .= '               <label>Statut Bloquant ? <input type="checkbox"  name="form_isBlock" value="1" /></label>';
